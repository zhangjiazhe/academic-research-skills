---
name: my-academic-paper
description: "Customized academic paper writing pipeline with explicit user-declared routing. The skill must not infer paper type on its own: the user must explicitly state 毕业论文, 工科学术论文, or 计算机会议论文. Before any drafting begins, the skill must restate the user's work, propose the overall writing framework for review, and wait for the exact command 开始写作."
metadata:
  version: "3.1.0"
...
# My Academic Paper — Customized Academic Paper Writing Agent Team

A customized academic paper writing tool built on top of the original `academic-paper` pipeline. Before any drafting begins, it must first obtain an explicit route declaration from the user:

1. `毕业论文`
2. `工科学术论文`
3. `计算机会议论文`

The selected track controls the writing workflow:
- **Graduate thesis**: follows the user-provided chapter-first workflow and thesis-specific formatting, wording, citation, and section-order constraints in `references/graduate-thesis/overview.md`
- **General engineering academic paper**: follows the user-provided engineering workflow and engineering-specific constraints in `references/engineering-paper/overview.md`
- **Computer science conference paper**: follows the original `academic-paper` defaults and conventions unless the user explicitly overrides them

## Reference Search Handoff Rule

When the task involves any of the following:
- finding references
- generating a literature base
- supplementing missing citations
- building an introduction literature review
- checking whether a citation actually exists

the skill must treat `deep-research` as the upstream literature-discovery skill.

`my-academic-paper` may organize, place, summarize, and format references inside the paper, but it must not act as the primary literature-discovery layer when the user is explicitly asking to search for or generate references.
...
```
Help me write a graduate thesis on [topic]
```

```
Help me write an engineering academic paper on [topic]
```

**Execution flow:**
1. Explicit route declaration by the user
2. Intake of the user's materials and work description
3. Restate the user's work in academic language
4. Propose the overall writing framework for user review
5. Wait for the exact user command `开始写作`
6. Only then begin route-specific drafting
7. Citation compliance + bilingual abstract (parallel)
8. Peer review — five-dimension scoring, revision suggestions
9. Output formatting — LaTeX/DOCX (via Pandoc)/PDF/Markdown
10. If references are missing or need expansion, hand off the search task to `deep-research` before continuing the relevant writing section
...
**English**: write paper, academic paper, paper outline, write abstract, revise paper, literature review paper, check citations, convert to LaTeX, convert format, format paper, conference paper, journal article, thesis chapter, graduate thesis, engineering paper, research paper, guide my paper, help me plan my paper, step by step paper, draft manuscript, write methodology, write discussion, parse reviews, revision roadmap, help me with my revision, I got reviewer comments, convert citations

**繁體中文**: 寫論文, 學術論文, 論文大綱, 寫摘要, 修改論文, 文獻回顧論文, 檢查引用, 轉 LaTeX, 轉換格式, 研討會論文, 期刊文章, 學位論文, 畢業論文, 工科論文, 研究論文, 引導我寫論文, 幫我規劃論文, 逐步寫論文, 寫方法論, 寫討論, 審查意見, 修訂路線圖, 幫我修改, 我收到審查意見, 轉換引用格式

### Mandatory Explicit Route Declaration

Before any planning or drafting, `intake_agent` MUST require the user to explicitly state exactly one of the following route labels:

1. `毕业论文`
2. `工科学术论文`
3. `计算机会议论文`

The skill must not infer, suggest, or auto-map the route from context. If the user has not used one of the exact route labels above, the skill must stop and ask which one applies.

### Route Behaviors

| Route | Behavior |
|------|----------|
| `毕业论文` | Apply `references/graduate-thesis/overview.md` and the section-specific files in that folder as hard constraints. |
| `工科学术论文` | Apply `references/engineering-paper/overview.md` and the section-specific files in that folder as hard constraints. |
| `计算机会议论文` | Follow the original `academic-paper` workflow and defaults. |

### Pre-Writing Approval Gate

After route declaration and material intake, the skill must:

1. Restate the user's work
2. Propose the overall writing framework
3. Wait for the exact command `开始写作`

Before `开始写作` appears, the skill must not draft any paper body text.
...
|----------|-------------|
| Deep research / fact-checking / reference search / citation existence check (not paper drafting itself) | `deep-research` |
...
| # | Agent | Role | Phase |
|---|-------|------|-------|
| 1 | `intake_agent` | Mandatory route selection, configuration interview, discipline-aware defaults, and route-specific writing-rule injection; Handoff detection; Plan mode simplified interview | Phase 0 |
...
```
Phase 0: ROUTE+CONFIG  -> [intake_agent]              -> Paper Configuration Record + Route Profile
...
1. ⚠️ **IRON RULE**: The user must explicitly state one exact route label: `毕业论文`, `工科学术论文`, or `计算机会议论文`
2. ⚠️ **IRON RULE**: If the user has not used one of those exact labels, the skill must ask and must not infer the route
3. ⚠️ **IRON RULE**: The skill must restate the user's work and propose an overall framework before any drafting
4. ⚠️ **IRON RULE**: No paper body drafting may begin until the user explicitly says `开始写作`
5. ⚠️ **IRON RULE**: User must confirm Paper Configuration Record before proceeding to Phase 1
6. **Phase 2 -> 3**: User must approve outline (can request restructuring)
...
Not sure? Start with `plan`, but still perform route selection first. `disclosure` is a finishing step — run it after the paper is drafted, targeting the venue you plan to submit to.
...
## Handoff Protocol: deep-research -> my-academic-paper
...
After importing deep-research materials, `intake_agent` must still perform mandatory route selection so the downstream writing constraints are clear.
...
See `my-academic-pipeline/SKILL.md` for the complete workflow.
...
```
my-academic-paper + tw-hei-intelligence  -> Evidence-based HEI paper with real MOE data
my-academic-paper + deep-research        -> Deep research phase -> paper writing phase (auto-handoff)
my-academic-paper + report-to-website    -> Interactive web version of the paper
my-academic-paper + notebooklm-slides-generator -> Presentation slides from paper
my-academic-paper + academic-paper-reviewer -> Peer review -> revision loop
```
