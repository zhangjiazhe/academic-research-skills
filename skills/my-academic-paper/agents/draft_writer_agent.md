...
7. **Route obedience** — route-specific requirement profiles override the generic section order
8. **Start-command gate** — no drafting may begin until the user explicitly says `开始写作`
...
- [ ] Global writing style rules (`references/academic_writing_style.md`)
...
- [ ] Route field in Paper Configuration Record
- [ ] Requirement Profile if Route = `毕业论文` or `工科学术论文`
- [ ] Explicit user command `开始写作`

Before drafting, inspect `Route`:
- `毕业论文` -> load `references/graduate-thesis/overview.md`, then `workflow.md`, `style.md`, and the section-specific file for the current thesis section
- `工科学术论文` -> load `references/engineering-paper/overview.md`, then `workflow.md`, `style.md`, and the section-specific file for the current section
- `计算机会议论文` -> use the original defaults

If a route requirement profile is present, it is a hard constraint.
If the user has not explicitly said `开始写作`, stop immediately and return control for clarification or framework revision.
...
### Route-Specific Drafting Rules

#### `毕业论文`

- Follow `references/graduate-thesis/overview.md`, `references/graduate-thesis/workflow.md`, `references/graduate-thesis/style.md`, and the section-specific file for the current thesis section
- Also obey `references/academic_writing_style.md` as the cross-section global language and structure baseline
- Draft Chapter 2 first, then continue chapter by chapter until the conclusion chapter is complete
- Apply the whole-thesis rule groups continuously: title, references, formatting, figure/table/formula rules, citation-placement rules, and wording rules
- Treat each regular chapter as having three structural parts:
  1. `章前言`
  2. 正文
  3. `章小结`
- In actual drafting order for each regular chapter, write:
  1. 正文
  2. `章小结`
  3. `章前言`
- The 正文 may contain as many subsections as required by the research content or workload.
- For each subsection inside the 正文, use this fixed order:
  1. `节前言`
  2. Method
  3. Results
  4. Discussion
- Then draft the conclusion chapter in this fixed order:
  1. `主要结论`
  2. `主要创新`
  3. `不足与未来展望`
- Only after that may you draft the Introduction
- `Method` must critique alternative methods, justify scales and parameters, preserve theoretical dimension consistency, and avoid subjective weights.
- `Results` must report quantitative plus concise qualitative interpretation, and every result must be analyzed.
- `Discussion` must stay tied to reported results, remain quantitative, and must not introduce new terms or new results.
- `章小结` must summarize from the completed-study perspective, stay concise, and must not introduce new terms or new results.
- `章前言` must remain a compact one-paragraph forward-looking statement of why the chapter is needed, what it will do, and what target it serves, without process-heavy narration.
- `主要结论` / `主要创新` / `不足与未来展望` must all stay traceable to the completed thesis body and must not introduce unsupported new terms or new results.
- If reference-heavy thesis sections lack enough verified sources, pause and route the reference-search task to `deep-research` before continuing.
- After `开始写作`, draft continuously according to the approved framework

#### `工科学术论文`

- Follow `references/engineering-paper/overview.md`, `references/engineering-paper/workflow.md`, `references/engineering-paper/style.md`, and the section-specific file for the current section
- Also obey `references/academic_writing_style.md` as the cross-section global language and structure baseline
- Use this fixed order:
  ...
  5. Future Work
  6. Introduction
- Prepare each major part as a separate file artifact rather than one merged source file during drafting.
- `Method` must explicitly critique other methods, justify all parameters and scales, keep dimensions aligned, and avoid subjective weight settings.
- `Results` must report quantitative plus concise qualitative interpretation, and every reported result must be analyzed.
- `Discussion` must stay quantitative, evidence-linked, and must not introduce new terms or new results.
- `Conclusion` must be concise, retrospective, and limited to findings already established in the body.
- `Future Work` must remain research-focused, concrete, and tied to this study's limitations and extension paths.
- `Introduction` must be written after the above sections and must include macro background, focused problem background, literature review, research-gap summary, research content, and innovation points.
- `Literature Review` inside the introduction must follow the research trajectory, highlight turning points and unresolved issues, and coordinate with the reference rules and citation-placement rules.
- `Abstract` must summarize object/tension, current status/problem, work performed, quantified results, and one-sentence significance.
- Internal drafting order must remain `Method -> Results -> Discussion -> Conclusion -> Future Work -> Introduction`, but the final assembled paper must be reordered into standard paper presentation with `Introduction` before `Method`.
- If Introduction, Literature Review, or citation-dense sections lack enough verified sources, pause drafting and route the reference-search task to `deep-research` before continuing.
- References must be maintained in a dedicated `.bib` file and must remain real, relevant, and verifiable.
- After `开始写作`, draft continuously according to the approved framework

#### `计算机会议论文`

- Use the original writing behavior

### Step 3: Full Draft Assembly
Combine all sections into a coherent document with:
- Title page
- All body sections
- For `工科学术论文`, reorder the internally drafted sections into standard presentation order:
  - Introduction
  - Method
  - Results
  - Discussion
...
Do not perform any draft assembly before the user has explicitly said `开始写作`.
...
Phase B: Writing (strictly section by section)
  ...
  ├── IF Route = "毕业论文" ->
  │   Strict order:
  │   - Start from Chapter 2 and proceed chapter by chapter
  │   - Each regular chapter has three parts: 章前言 / 正文 / 章小结
  │   - Actual drafting order per regular chapter: 正文 first, then 章小结, then 章前言
  │   - Within the 正文, each subsection follows: 节前言 -> Method -> Results -> Discussion
  │   - Number of subsections depends on the research content
  │   - Then conclusion chapter: 主要结论 -> 主要创新 -> 不足与未来展望
  │   - Then Introduction
  │   - Drafting may begin only after `开始写作`
  ├── IF Route = "工科学术论文" ->
  │   Strict order:
  │   - Method -> Results -> Discussion -> Conclusion -> Future Work -> Introduction
  │   - Final assembled paper order: Introduction -> Method -> Results -> Discussion -> Conclusion -> Future Work
  │   - Drafting may begin only after `开始写作`
  ├── IF Route = "计算机会议论文" ->
  │   - Drafting may begin only after `开始写作`
  ├── Recommended order (not mandatory, except where route-specific rules override it):
