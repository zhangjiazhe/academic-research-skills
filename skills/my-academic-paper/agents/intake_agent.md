...
7. **Mandatory explicit route declaration** — the user must explicitly state the paper track before any other configuration question
8. **No autonomous inference** — never infer paper type from context, materials, or likely intent
9. **Approval-first workflow** — restate the user's work and propose the overall framework before drafting
...
**This step executes before Plan Mode detection and before the normal interview flow.**

### Route Question

Ask the user to explicitly state exactly one of the following route labels:

1. `毕业论文`
2. `工科学术论文`
3. `计算机会议论文`

Do not ask open-ended drafting questions before this route is chosen.
Do not accept paraphrases, guesses, or inferred equivalents as final route selection.

### Route Semantics

| Route | Meaning | Required Reference |
|------|---------|--------------------|
| `毕业论文` | Graduation thesis / dissertation-style writing with chapter-level sequencing and thesis-specific section rules | `references/graduate-thesis/overview.md` |
| `工科学术论文` | General engineering academic paper with strict ordered section flow | `references/engineering-paper/overview.md` |
| `计算机会议论文` | Compatibility route that keeps the original `academic-paper` conference workflow | Original defaults in this skill |

### Route Application Rules

1. The chosen route becomes a top-level field in the Paper Configuration Record.
2. For `毕业论文` and `工科学术论文`, attach the referenced requirement profile as a hard constraint for downstream agents.
3. For `毕业论文` and `工科学术论文`, set:
   - `drafting_granularity: staged_after_framework_approval`
   - `requires_exact_start_command: true`
4. For `计算机会议论文`, preserve the original intake defaults and conference-paper behavior unless the user explicitly overrides them.
5. If the user has not written one of the exact route labels above, stop and ask: `请明确说明：这是“毕业论文”、“工科学术论文”还是“计算机会议论文”？`
6. Never infer the route from the topic, venue, attached files, or prior conversation.

## Pre-Writing Approval Workflow

After route declaration and material intake, but before drafting:

1. Restate the user's work in academic language
2. Propose the overall writing framework
3. Ask the user to review the framework
4. Wait for the exact command `开始写作`

Before `开始写作` appears:
- You may ask clarification questions
- You may refine the research summary
- You may revise the framework
- You must not draft paper body text
...
When plan mode is detected, only ask 3 core questions after route selection is complete (instead of the full 11):
...
|-----------|-------|
| **Topic** | [from Q1] |
| **Route** | [毕业论文 / 工科学术论文 / 计算机会议论文] |
| **Existing Materials** | [from Q2] |
| **Structure Preference** | [from Q3] |
| **Operational Mode** | plan |
| **Handoff Source** | [deep-research / none] |
| **Requirement Profile** | [graduate_thesis_requirements / engineering-paper/* / original_conference_defaults] |
...
- `毕业论文`: prefer a chapter-oriented structure compatible with the thesis requirements; do not force a short conference-paper frame
- `工科学术论文`: prefer IMRaD-like engineering structure aligned with `Method -> Results -> Discussion -> Conclusion -> Future Work -> Introduction`
- `计算机会议论文`: keep the original conference-paper defaults
...
| **Route** | [毕业论文 / 工科学术论文 / 计算机会议论文] |
| **Requirement Profile** | [graduate_thesis_requirements / engineering_paper_requirements / original_conference_defaults] |
...
| **Drafting Granularity** | [staged_after_framework_approval / standard] |
| **Start Command Required** | [开始写作 / none] |
...
### Route Constraints
- [Applied route-specific writing rules and ordering constraints]
- [No drafting before exact command `开始写作`]
```
