...
6. **Route obedience** — if the Paper Configuration Record specifies a writing route, that route overrides the generic default order
7. **Framework-before-drafting** — your outline is for user review; it is not permission to start writing
...
## Route-Specific Hard Overrides

If the Paper Configuration Record includes a `Route` field, apply the following hard overrides before using the generic pattern defaults:

### `毕业论文`

- Respect `references/graduate-thesis/overview.md`, `references/graduate-thesis/workflow.md`, and the section-specific files in `references/graduate-thesis/`
- Build the working order so that Chapter 2 starts first, then continue chapter by chapter through the conclusion chapter, and only then outline the Introduction
- Keep the whole thesis under the shared rule groups for title, references, formatting, figures/tables/formulas, citation placement, and wording
- Treat each regular chapter as having three structural parts:
  1. `章前言`
  2. 正文
  3. `章小结`
- In actual writing order, the 正文 must be outlined first, then `章小结`, and only then `章前言`
- The 正文 may contain as many subsections as required by the research content
- For each subsection inside the 正文, the internal order must be:
  1. `节前言`
  2. Method
  3. Results
  4. Discussion
- Treat `章前言` as the final compact chapter-introduction block written after the whole chapter body and `章小结`
- After the regular chapters, build the conclusion chapter in this order:
  1. `主要结论`
  2. `主要创新`
  3. `不足与未来展望`
- Only after the above may the Introduction be outlined

### `工科学术论文`

- Respect `references/engineering-paper/overview.md`, `references/engineering-paper/workflow.md`, and the section-specific files in `references/engineering-paper/`
- The section order is fixed:
  ...
  5. Future Work
  6. Introduction
- Each major part must be designed as a separate file artifact rather than a single merged draft file.

### `计算机会议论文`

- Use the original conference-paper defaults
...
If `Route = 毕业论文` or `Route = 工科学术论文`, the route determines the actual writing order even when the underlying structure label remains IMRaD-like.
...
For `毕业论文` and `工科学术论文`, headings and ordering must remain compatible with the route requirement profile.
For `工科学术论文`, the outline must also mark each major section as a separate file deliverable.
For `毕业论文`, the outline must make clear:
- the chapter-level three-part structure: `章前言` / 正文 / `章小结`
- the actual writing order: 正文 -> `章小结` -> `章前言`
- the subsection-level body structure: `节前言` -> Method -> Results -> Discussion
- that the number of body subsections is determined by the research content rather than a fixed template

### Approval Gate

The outline produced at this stage is a review draft for user approval.
It must be followed by user review and possible revision.
It must not trigger drafting on its own.
Only the exact user command `开始写作` allows transition to drafting.
...
├── Route = "毕业论文" ->
│   Use thesis-specific chapter workflow:
│   - Start from Chapter 2
│   - Write chapter by chapter until the conclusion chapter is complete
│   - Each regular chapter has three parts: 章前言 / 正文 / 章小结
│   - Actual writing order per regular chapter: 正文 first, then 章小结, then 章前言
│   - Within the 正文, each subsection follows: 节前言 -> Method -> Results -> Discussion
│   - Number of subsections is driven by the research content
│   - Then conclusion chapter: 主要结论 -> 主要创新 -> 不足与未来展望
│   - Then Introduction
├── Route = "工科学术论文" ->
│   Use fixed engineering order:
│   - Method -> Results -> Discussion -> Conclusion -> Future Work -> Introduction
├── Route = "计算机会议论文" ->
│   Prefer Conference pattern and original defaults
