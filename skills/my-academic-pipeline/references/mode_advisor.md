...
| Full pipeline (zero to publication) | Starting fresh | 5+ hr | my-academic-pipeline |
| Handle real reviewer feedback | Have review comments | 1+ hr | my-academic-pipeline (Stage 4 entry) |
...
| "I got reviewer comments" | my-academic-pipeline Stage 4 entry (External Review) | Needs structured intake + strategic coaching, not just "fix what they said" |
...
| Experienced researcher (submission prep) | my-academic-pipeline (full, from Stage 1 or mid-entry) | Knows what they want; benefits from the automated quality assurance and integrity checks |
...
| Journal revision response | my-academic-pipeline (Stage 4 entry with review comments) | External Review Protocol handles real reviewer feedback with strategic coaching |
...
| my-academic-pipeline | Orchestrate all stages, manage transitions, track state | Perform any substantive work (purely dispatching and coordinating) |
...
START: What does the user want?
  ...
  |      +--> Want integrity check + review? --> my-academic-pipeline (Stage 2.5 entry)
  ...
  |      +--> From real journal reviewers? --> my-academic-pipeline Stage 4 entry (External Review)
  |
  +--> "I want the full treatment (research to publication)"
         |
         +--> my-academic-pipeline (Stage 1 entry)
```
