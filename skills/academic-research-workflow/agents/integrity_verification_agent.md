...
#### A-1. Structured Provenance Verification — NEW

This is a verification backend for references already present in the paper. It is not a new literature-discovery step.

Before WebSearch-based investigation, use `nature-academic-search` / CrossRef / PubMed / arXiv / DOI lookup to verify that each existing reference resolves to real structured metadata:

| Reference identifier | Preferred verification route |
|----------------------|------------------------------|
| DOI | CrossRef DOI metadata + DOI resolution / publisher page |
| PMID / PMCID | PubMed / NCBI metadata |
| arXiv ID | arXiv metadata |
| Title + first author + year | `nature-academic-search` multi-source lookup, then CrossRef/PubMed/arXiv where applicable |
| Publisher URL | Publisher page metadata + DOI/CrossRef if available |

Classification:

| Result | Meaning | Action |
|--------|---------|--------|
| `STRUCTURED_VERIFIED` | Structured metadata exists and title/authors/year/venue match | Proceed to A2 bibliographic accuracy |
| `STRUCTURED_MISMATCH` | Identifier resolves, but metadata does not match the reference | SERIOUS; possible DOI misdirection or citation corruption |
| `STRUCTURED_NOT_FOUND` | No structured source resolves the reference | Proceed to A0/A1 for additional investigation |
| `UNVERIFIED_NO_PROVENANCE` | No identifier, query, tool result, source file, or publisher/database trail exists | Treat as NOT VERIFIED and exclude/correct |

Every reference must have a structured provenance row unless the source type is outside structured databases (e.g., local archival material, legal documents, internal reports). For those exceptions, record the official source path or user-provided file as the provenance.
...
```
For each reference:
1. First use structured lookup where possible (`nature-academic-search`, CrossRef, PubMed, arXiv, DOI)
2. If structured lookup fails or the source is outside structured databases, WebSearch: author name + paper title + year
2. Confirm the reference actually exists
3. Compare search results with citation details

Determination:
- VERIFIED: Found credible source (structured metadata, publisher page, DOI, Google Scholar) confirming reference exists with matching bibliographic details
...
#### A2 Enforcement Rule
Every reference MUST have an audit trail entry showing:
1. The structured lookup query/identifier used, or the WebSearch query when structured lookup is unavailable
2. The top structured record / publisher URL / search result URL
...
2. Use structured lookup (`nature-academic-search`, CrossRef, PubMed, arXiv, DOI) plus WebSearch when needed to find the original source text
3. If source is behind paywall, note as UNVERIFIABLE_ACCESS

Priority:
- `nature-academic-search` / CrossRef / PubMed / arXiv / DOI resolution / publisher official website
- Semantic Scholar / Google Scholar / ERIC / Scopus
...
[List the verification process for each reference and originality comparison: structured lookup / search terms -> results -> determination]
```
...
2. **Verification source priority order**:
   - Level 1: `nature-academic-search` structured lookup / CrossRef / PubMed / arXiv / DOI metadata / publisher official website
   - Level 2: Semantic Scholar / Google Scholar / ERIC / Scopus
...
| Accuracy | Every determination must be supported by structured metadata, publisher/database evidence, user-provided source files, or documented WebSearch evidence |
