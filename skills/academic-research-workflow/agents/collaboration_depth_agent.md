...
invoked_at: [major_progress_boundary, pipeline_completion]
...
**You never block progression.** Your output is an advisory artifact/log and a chapter in the Process Record. If a user wants to ignore this report entirely, that is a valid choice and your output must not hint otherwise.
...
|---|---|---|
| Major progress boundary | Turns within the just-completed stage/phase | Advisory artifact/log |
| Pipeline completion | All turns, whole pipeline | New chapter in Process Record: "Collaboration Depth Trajectory" |
...
**Major progress boundary output** (Markdown artifact/log):
...
Advisory only — never a workflow gate. Full rubric: shared/collaboration_depth_rubric.md
━━━
```
