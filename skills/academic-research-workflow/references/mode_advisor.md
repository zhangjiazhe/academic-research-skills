...
| Write a paper | Have research done | 2+ hr | my-academic-paper full |
| Plan a paper step by step | Have RQ, need structure | 1+ hr | my-academic-paper plan |
| Fix citations | Have draft | 30 min | my-academic-paper citation-check |
| Convert format | Have final draft | 15 min | my-academic-paper format-convert |
...
| Full pipeline (zero to publication) | Starting fresh | 5+ hr | academic-research-workflow |
| Handle real reviewer feedback | Have review comments | 1+ hr | academic-research-workflow (Stage 4 entry) |
...
|-----------|-------------------|-----|
| "Write me a paper on X" | deep-research first, THEN my-academic-paper | Writing without research produces shallow papers with unsupported claims |
| "Review my paper" (but no draft exists) | my-academic-paper plan mode | They need to write first, not review |
| "Check my citations" (but paper isn't done) | my-academic-paper full mode | Finish writing first, then check citations as a separate pass |
| "I need a systematic review" | deep-research systematic-review mode | NOT my-academic-paper lit-review structure (different methodology: PRISMA vs narrative) |
| "Just give me a quick paper" | deep-research quick + my-academic-paper full | Quick research is fine, but paper writing still needs the full mode for quality |
| "Format my paper as APA" | my-academic-paper format-convert mode | Not a rewrite; purely formatting transformation |
| "I got reviewer comments" | academic-research-workflow Stage 4 entry (External Review) | Needs structured intake + strategic coaching, not just "fix what they said" |
...
|-----------|---------------------|-----------|
| Graduate student (first paper) | deep-research socratic -> my-academic-paper plan -> full pipeline | Socratic mode builds research thinking; plan mode structures the paper incrementally; pipeline ensures quality gates |
| Experienced researcher (submission prep) | academic-research-workflow (full, from Stage 1 or mid-entry) | Knows what they want; benefits from the automated quality assurance and integrity checks |
...
| Journal revision response | academic-research-workflow (Stage 4 entry with review comments) | External Review Protocol handles real reviewer feedback with strategic coaching |
| Conference paper (short deadline) | deep-research quick -> my-academic-paper full (conference type) | Compressed timeline; quick research + full writing with conference structure |
| Thesis chapter | deep-research full -> my-academic-paper full | Each chapter treated as a standalone paper; full depth needed |
| Policy brief | deep-research quick -> my-academic-paper full (policy_brief type) | Evidence-based but concise; quick research sufficient for policy scope |
...
| my-academic-paper | Write papers, revise papers, format documents, check citations | Conduct original research, review papers (as reviewer), verify integrity |
...
| academic-research-workflow | Orchestrate all stages, manage transitions, track state | Perform any substantive work (purely dispatching and coordinating) |
...
START: What does the user want?
  ...
  |      |
  |      +--> Have research/literature ready? --> my-academic-paper (plan or full)
  |      +--> No research done yet? --> deep-research FIRST, then my-academic-paper
  |      +--> Want full quality assurance? --> academic-research-workflow (from Stage 1)
  |
  +--> "I want someone to review my paper"
  |      |
  |      +--> Have a complete draft? --> academic-paper-reviewer full
  |      +--> Want integrity check + review? --> academic-research-workflow (Stage 2.5 entry)
  |      +--> No draft yet? --> my-academic-paper first
  |
  +--> "I need to revise based on feedback"
  |      |
  |      +--> From AI reviewers (pipeline)? --> Continue pipeline (Stage 4)
  |      +--> From real journal reviewers? --> academic-research-workflow Stage 4 entry (External Review)
  |
  +--> "I want the full treatment (research to publication)"
         |
         +--> academic-research-workflow (Stage 1 entry)
```
...
| Stage 2 (WRITE) | RQ Brief + Verified Source Corpus | Stage 1 | Full pipeline from Stage 2 |
...
| Running pipeline for a 1-page abstract | Overhead far exceeds benefit | Use my-academic-paper full directly |
